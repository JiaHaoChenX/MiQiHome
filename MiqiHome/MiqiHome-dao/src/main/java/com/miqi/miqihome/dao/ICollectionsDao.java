...
public interface ICollectionsDao {
    ...
    * 新增
    * */
    int insert_collection(@Param("uiId") String uiId,@Param("haiId") String haiId);
    ...
    * 删除
    * */
    int delete_collection(@Param("uiId") String  uiId,@Param("haiId") String haiId);

    /**
     *  发布人：丁火钦
     *  添加：根据用户id查询该用户是否收藏了该房间
     */
    int select_ui_collection(@Param("uiId") String uiId,@Param("haiId") String haiId);

    /**
     *  发布人：丁火钦
     *  作用  ：根据用户id查询该用户收藏的房间
     */
    List<Homeallinformation> select_ui_collection_list(@Param("uiId") String uiId);
}
