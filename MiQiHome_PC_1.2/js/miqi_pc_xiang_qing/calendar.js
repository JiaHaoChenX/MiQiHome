...
Y.TripCalendar = Y.extend(Calendar, Y.Base, {
    ...
    initializer: function() {
        ...
        //console.log(this._minDateCache);
        this.flow = this.get('count');  
        this._sDiscount=this.get('sDiscount'); 
        ...
        this.set('maxDate',this.initMaxDate());
    ...
    initMaxDate:function(){
        ...
        var day = new Date(nextYear,nextMonth,0).getDate();
        var nowDate = new Date(nextYear,nextMonth);
        var dateStr = nextYear+'-'+nextMonth+'-'+day;
        console.log(dateStr);
        return dateStr;
    },
   ...
   getSelectedDatee: function() {
    ...
    _setWidth: function() {
        (function(that, boundingBox, contentBox) {
            //boundingBox.setStyle('width',200);
            if(IE !== 6) return this;
        ...
        switch(true) {
            ...
            case !this.boundingBox.hasClass('calendar-bounding-box-style') && !!this.get('finalTriggerNode'):
                ...
                if(finalTriggerNode && this.get('isAutoSwitch')) {
                	//判断起始日期是否大于结束日期，大于结束日期则结束日期清空	2015-01-05	zhaopengwei start
                	if((new Date(this.get('startDate'))) >= (new Date(this.get('endDate')))){
                		finalTriggerNode.set('value','');
                	}
                	//End
                    finalTriggerNode._node.focus();
                }
                break;
            default:
                this.set('selectedDate', v);
        ...
        (function(that, currentNode) {
            ...
            setTimeout(function() {
                var iLeft              = currentNode.getX(),
                    ...
                    //日历位置固定 ludi 2015-07-01
                    coordinateX =that.get('coordinateX'),
                     ...
                     */
                    iMaxTop            = currentNode.get('winHeight') * 1; 
                	var iWidth=522;
                	if( that.flow==1)
                	{
                		iTop+=252.2;
                ...
                that.boundingBox.setStyles({
                    top : strBODY != 'body' ? dTop : iTop,
                    left: strBODY = ".date" ? dLeft : iLeft,
                    width:iWidth
    ...
    _DELEGATE: {
        ...
        'click': function(e) {
            ...
            switch(!0) {
                ...
                case !!target.getAttribute('data-date') && !target.hasClass('disabled'):
                	//终止日期点击起始日期时设置为无反应	2015-01-05	zhaopengwei  start
                	if(this.boundingBox.hasClass('calendar-bounding-box-style')){
                		 if(date == this.get('startDate')) return;
                	}
        ...
        'triggerNodeClick': function(e) {
        	var citypinyin = document.getElementById("my_p_y").value;
        	var user = getCookie('MAYIUID');
			if(citypinyin=="beijing"&&user==null){
    ...
    _initCalendarHTML: function() {
        var calendar_template                     = {};
            ...
            var template=Calendar.CALENDAR_TEMPLATE;
            var min_template=Calendar.MIN_DAY_TEMPLATE;
            var minday =this.get('minday');
            
            if(minday==''){
            	template=template.replace('{NUM}',minday);
            }else{
            	min_template = min_template.replace('{NUM}',minday)
            	template=template.replace('{NUM}',min_template);
            }
            var discount = '';
        ...
        return toHTML(template, calendar_template);       
    },
  ...
  var  datename = {
        ...
        var price=[];
        //2015-07-02 ludi
        var calPrice =this.get('calPrice');
        Calendar.INVOKE_NO++;
        if(Calendar.INVOKE_NO%2 ==0){
        	Calendar.START_DATE=iYear+ '-' +iMonth+'-'+'01';
        }
        
    	if(calPrice!=1 && Calendar.INVOKE_NO%2 ==0){
    		
    		var url=this.get('ctx')+"/room/getPrice";
    		
    		var roomid =this.get('roomid');
    		var startDate =Calendar.START_DATE;
    		jQuery.post(url, {
                roomid: roomid,startDate:startDate}, function(data) {
                	var jsonData = eval(data); 
            	    Calendar.CAL_PRICE_JSON = jsonData.calPriceJson;
    	...
    	tDate.setMinutes(0);
    	tDate.setSeconds(0);
    	for(var i=0;i<Calendar.CAL_PRICE_JSON.length;i++){
    		
			var nDate = new Date(Calendar.CAL_PRICE_JSON[i].date.replace(/-/g,"/"));
			var nYear=nDate.getFullYear();
			var nMonth=nDate.getMonth()+1;
			var nDay=nDate.getDate();
			if(nYear==iYear && nMonth==iMonth){
				
				//console.log(tDate+'||'+nDate+'===='+(Date.parse(tDate)<=Date.parse(nDate)));
				
				if((Calendar.CAL_PRICE_JSON[i].isRent==0||Calendar.CAL_PRICE_JSON[i].stock==0)&& (Date.parse(tDate)<=Date.parse(nDate))){
        ...
        for(var i = 0; i < rows; i++) {
            ...
            for(var j = 0; j <= 6; j++) {
                ...
                if(days!="" && calPrice!=1 && price.length>0){
                	days=price[days-1];
                }
                ...
                var sdate=
					...
					if(sdate.indexOf('class=""')==-1)
					{
						var sday = sdate.match(/class="(\w{0,})"/)[1]
						sday =	datename[sday]
						sdate=sdate.replace(/\d{0,}<\/a>/,sday+"</a>");
					}
					
					if(sdate.indexOf('无房')!=-1){
						var begin_index =sdate.indexOf('无房');
						var end_index =sdate.indexOf('</a');
						sdate=sdate.substring(0,begin_index-1)+' style="background-color:#fff;color:grey;font-weight:100">无房'+sdate.substring(end_index,sdate.length);
					} 
...
},
{
    ...
    CALENDAR_TEMPLATE: '<div id="{bounding_box_id}" class="calendar-bounding-box">' +
                            '<div class="container">' +
                                ...
                                '<div class="content-box">' +
                                    '<div class="arrow">' +
                                        '<div class="close-btn {delegate_click}"  id="clearSelect">关闭</div>' +
                                        '<span class="prev-btn {delegate_click}" title="上月"></span>' +
                                        '<span class="next-btn {delegate_click}" title="下月"></span>' +
                                    '</div>' +
                                    '<div class="date-box">' +
                                        '{date_template}' +
                                    '</div>' +
                                    
                                    '<div class="date-box" style="text-align: right;display:block;">' +
                                    	'<div class="inner" >{NUM}'+ 
                                		'{DISCOUNT}' +
                                    '</div>' +
    ...
    DAY_TEMPLATE: '<td data-date="{date}" class="{disabled}">' +
                        '<a id="id-{date}" href="javascript:;" class="{date_class}">' +
                            '{day}' +
                        '</a>' +                    
                    '</td>',
    ...
    MIN_DAY_TEMPLATE:'<h5 style="font-size: 12px;line-height:30px;margin-right: 8px;">此房源最少入住{NUM}晚</h4></div>',
