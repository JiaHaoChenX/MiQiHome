	...
	function getIndexDay(){
		...
		for( var k = 0;k<calendarDate.days;k++ ){
			...
			for( var d in calendarDate.opt.data ){
				...
				var s=calendarDate.opt.data[d].dataState;
				//console.log(s);
				flag = checkDate(nowDate,dataDate);
				if( flag ){
					if(s=="无房"){
					riliHtml += '<div class="ht-rili-td ht-rili-onclick" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" data-money="'+ calendarDate.opt.data[d].data +'">&yen;'+ calendarDate.opt.data[d].data +'</span><span class="ht-rili-money2">'+ calendarDate.opt.data[d].dataState +'</span></div>';
					}else{
						riliHtml += '<div class="ht-rili-td ht-rili-onclick ht-rili-td-active" data-date="'+ dataDateStr +'"><span class="ht-rili-day">'+ (k + 1) +'</span><span class="ht-rili-money" data-money="'+ calendarDate.opt.data[d].data +'">&yen;'+ calendarDate.opt.data[d].data +'</span><span class="ht-rili-money2">'+ calendarDate.opt.data[d].dataState +'</span></div>';
					}
